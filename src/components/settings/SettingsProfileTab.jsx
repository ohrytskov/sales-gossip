        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50" onClick={() => setShowEditUsername(false)}>
          ...
          >
            ...
            <div data-layer="Frame 48097040" className="Frame48097040 w-[566px] h-16 left-0 bottom-0 absolute overflow-hidden">
              <div
                data-layer="Primary Button"
                className={`PrimaryButton h-10 px-5 py-2 left-[469px] bottom-[14px] absolute rounded-[56px] inline-flex justify-center items-center gap-2 ${!canSaveUsername ? 'bg-[#e5c0d1]' : 'bg-pink-700 cursor-pointer'}`}
                onClick={async () => {
                ...
                onClick={
                  descriptionSaving
                    ...
                    : async () => {
                        ...
                        const trimmed = String(descriptionDraft || '')
                          .trim()
                          .slice(0, 250)
                        try {
