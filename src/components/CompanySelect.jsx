...
const resolveLogoUrl = (logo) => {
  if (!logo) return null
  if (/^https?:\/\//i.test(logo) || /^\/\//.test(logo)) return logo
  if (logo.startsWith('/')) {
    try {
      if (typeof window !== 'undefined') return window.location.origin + logo
    } catch (e) {
      return logo
    }
  }
...
export default function CompanySelect({ value, onChange }) {
  ...
  const { data: brands } = useCompanies()
  const companies = Array.isArray(brands) ? brands : (brands ? Object.values(brands) : [])
  ...
  // repeat companies 3 times for demo purposes
  const displayedCompanies = []
  if (baseFiltered && baseFiltered.length) {
    for (let r = 0; r < 3; r++) {
      baseFiltered.forEach((c, idx) => displayedCompanies.push({ ...c, _displayKey: `${c.id}__${r}__${idx}` }))
    }
  }
    ...
    >
      ...
      <div className="flex items-center h-full px-4">
          ...
          {showCompanyDropdown ? (
            ...
            selectedCompany ? (
              selectedCompany.logo ? (
                <img src={resolveLogoUrl(selectedCompany.logo)} className="w-8 h-8 rounded-full border border-[#f2f2f4]" alt={selectedCompany.title || 'company'} />
              ) : (
                <img src={`https://placehold.co/32x32?text=${encodeURIComponent((selectedCompany.title && selectedCompany.title[0]) || '')}`} className="w-8 h-8 rounded-full border border-[#f2f2f4]" alt={selectedCompany.title || 'company'} />
              )
            ) : (
        ...
        <input
          ref={searchInputRef}
          value={searchTerm}
          onChange={(e) => { setSearchTerm(e.target.value); setShowCompanyDropdown(true); }}
          onFocus={() => setShowCompanyDropdown(true)}
          placeholder={selectedCompany?.title || (companies && companies.length ? 'Search companies' : 'Loading companies...')}
        ...
        <>
          { (baseFiltered && baseFiltered.length) ? (
              ...
              <div ref={listRef} className="p-4 flex-1 overflow-auto" style={{ overflowX: 'hidden' }} onWheel={(e) => {
                ...
                <div className="inline-flex flex-col justify-start items-start gap-4" style={{ minHeight: 0 }}>
                  {displayedCompanies.map((c) => (
                    ...
                    >
                      <img
                        data-layer="Dummy company logo"
                        className="w-8 h-8 rounded-full border border-[#f2f2f4]"
                        src={resolveLogoUrl(c.logo) || `https://placehold.co/32x32?text=${encodeURIComponent((c.title && c.title[0]) || '')}`}
                        alt={c.title || 'logo'}
                        onError={(e) => { try { e.currentTarget.src = `https://placehold.co/32x32?text=${encodeURIComponent((c.title && c.title[0]) || '')}` } catch (err) {} }}
                      />
            ...
            >
              ...
              <div data-layer="Label-text" className="LabelText w-44 left-[31px] top-[96px] absolute text-center justify-start text-[#0a0a19] text-sm font-medium font-['Inter'] leading-tight">No search results found for “{searchTerm}” </div>
            </div>
          ) : (
            <div
              ref={dropdownRef}
